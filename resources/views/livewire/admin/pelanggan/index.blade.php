        ...
        <div class="bg-white rounded-xl shadow-lg w-full max-w-lg p-6 max-h-screen overflow-y-auto">
            ...
            <form wire:submit="save">
                <div class="space-y-4">
                    ...
                    @if(!$pelangganId)
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Angka Meteran Air Awal (m³)</label>
                        <input type="number" wire:model="meter_awal" min="0" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Kosongkan jika dari 0">
                        <span class="text-xs text-slate-500">Angka meteran yang saat ini tercatat di rumah pelanggan.</span>
                        @error('meter_awal') <span class="text-red-500 text-xs block">{{ $message }}</span> @enderror
                    </div>
                    @endif
                    <div>
