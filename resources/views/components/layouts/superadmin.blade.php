...
<head>
    ...
    <title>{{ $title ?? 'Admin Dashboard - BILLPAMS' }}</title>
...
<body class="bg-slate-50 font-sans text-slate-900 antialiased flex h-screen overflow-hidden">
    ...
    <aside class="w-64 bg-slate-900 text-white flex flex-col hidden md:flex h-full shadow-lg">
        <div class="h-16 flex items-center px-6 border-b border-slate-800">
            <span class="text-xl font-bold tracking-tight text-blue-400">BILLPAMS</span>
            <span class="ml-2 text-xs font-medium text-slate-400 bg-slate-800 py-1 px-2 rounded">Tenant</span>
        </div>
        <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
            <a href="{{ route('superadmin.dashboard') }}" class="flex items-center px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-blue-600 hover:text-white transition {{ request()->routeIs('superadmin.dashboard') ? 'bg-blue-700 text-white' : 'text-slate-300 hover:bg-slate-800' }}">
                Dashboard Super Admin
            </a>
            <a href="{{ route('superadmin.tenant.index') }}" class="flex items-center px-4 py-2.5 rounded-lg text-sm font-medium transition {{ request()->routeIs('superadmin.tenant.*') ? 'bg-blue-700 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
                Daftar HIPPAM / Tenant
            </a>
            <a href="{{ route('superadmin.package.index') }}" class="flex items-center px-4 py-2.5 rounded-lg text-sm font-medium transition {{ request()->routeIs('superadmin.package.*') ? 'bg-blue-700 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }} mt-1">
        ...
        <div class="p-4 border-t border-slate-800">
            <div class="flex items-center text-sm mb-4 px-2">
                <div class="w-8 h-8 rounded-full bg-slate-700 flex items-center justify-center mr-3">
                    <span class="font-bold">{{ substr(Auth::user()->name ?? 'SA', 0, 1) }}</span>
                </div>
                <div class="flex-1 truncate">
                    <p class="font-medium truncate">{{ Auth::user()->name ?? 'Super Admin' }}</p>
    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <header class="bg-white shadow-sm h-16 flex items-center justify-between px-6 border-b border-slate-200 z-10">
            <div class="md:hidden">
                <span class="text-xl font-bold text-blue-700">BILLPAMS</span>
            </div>
